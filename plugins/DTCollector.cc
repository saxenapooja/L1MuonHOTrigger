...
void DTCollector::extractPrimitives(const edm::Event& ev, 
  ...
  for( int wheel = -2; wheel <= 2 ; ++wheel ) {    
	...
	for( int bx = bx_min; bx <= bx_max; ++bx) {	  
	  //	  std::unique_ptr<L1MuDTChambPhDigi> phi_segm_1( phiDigis->chPhiSegm1(    wheel, station, sector, bx) );
	  const L1MuDTChambPhDigi *phi_segm_1( phiDigis->chPhiSegm1(    wheel, station, sector, bx) );
	  const L1MuDTChambPhDigi *phi_segm_2( phiDigis->chPhiSegm2(    wheel, station, sector, bx) );
	  const L1MuDTChambThDigi *theta_segm( thetaDigis->chThetaSegm( wheel, station, sector, bx) );
