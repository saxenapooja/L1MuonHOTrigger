...
void L1DTTFTrackConverter::produce(edm::Event& ev, const edm::EventSetup& es) {
  ...
  std::auto_ptr<InternalTrackCollection>  convertedTracks (new InternalTrackCollection());
  ...
  edm::RefProd<DTTrackCollection> dttfProd =  ev.getRefBeforePut<DTTrackCollection>("input");
  ...
  for( int sp_wheel = -3 ; sp_wheel <= 3; ++sp_wheel ) {
	...
	for( int itrk = 1; itrk <=2; ++itrk ) {
	  ...
	  //std::unique_ptr<L1MuDTTrackCand> dttrk; //6_2_21
	  const L1MuDTTrackCand *dttrk;

	  if( itrk == 1 ) 
	    dttrk = dtTracks->dtTrackCand1(sp_wheel, sector, bx);
	  else            
	    dttrk = dtTracks->dtTrackCand2(sp_wheel, sector, bx);
